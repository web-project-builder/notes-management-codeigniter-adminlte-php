...
class Notes_category extends CI_Controller {
  	function __construct() {
	    parent::__construct();
	    $this->load->model("Notes_category_model"); 
		if(true==1){
			is_login(); 
      ...
      */
  	public function index() {   
  		if(CheckPermission("notes_category", "all_read,own_read")){
			$data["view_data"]= $this->Notes_category_model->get_data();
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			unset($data['save']);
			unset($data['id']);
			$this->Notes_category_model->updateRow('notes_category', 'notes_category_id', $this->input->post('id'), $data);
			$this->session->set_flashdata('message', 'Your data updated Successfully..');
      		redirect('notes_category');
		} else { 
			unset($data['submit']);
			unset($data['fileOld']);
			unset($data['save']);
			$data['user_id']=$this->user_id;
			$this->Notes_category_model->insertRow('notes_category', $data);
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Notes_category_model->Get_data_id($this->input->post('id'));
	...
	/**
      ...
      */
  	public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Notes_category_model->delete_data($value);		
		}
		redirect(base_url().'notes_category', 'refresh');
  	}
      ...
      */
  	public function delete_data($id) { 
		$this->Notes_category_model->delete_data($id);
	...
	/**
      ...
      */
  	public function ajx_data(){
		$primaryKey = 'notes_category_id';
		$table 		= 'notes_category';
		$columns 	= array(
array( 'db' => 'notes_category_id', 'dt' => 0 ),
array( 'db' => 'category_name', 'dt' => 1 ),
array( 'db' => 'notes_category_id', 'dt' => 2 )
);
		$joinQuery 	= '';
      ...
      */
  	public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where = " DATE_FORMAT(`notes_category`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND  DATE_FORMAT(`notes_category`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
		$data["view_data"]= $this->Notes_category_model->get_data($where);
